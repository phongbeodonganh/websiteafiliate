...
import { 
  ...
  MapPin, Share2, Code, Sparkles, Loader2, Wand2
} from 'lucide-react';
...
// Hàm hỗ trợ gọi Gemini API với cơ chế tự động thử lại (Exponential Backoff)
const fetchGeminiWithRetry = async (payload, retries = 3, delay = 1000) => {
  const apiKey = ""; // Canvas tự động inject API Key lúc runtime
...
const ArticleEditor = ({ article, onBack }) => {
  ...
  // States cho Gemini AI
  const [showAIPrompt, setShowAIPrompt] = useState(false);
  const [aiPrompt, setAiPrompt] = useState('');
  const [isGenerating, setIsGenerating] = useState(false);
  const [isGeneratingSlug, setIsGeneratingSlug] = useState(false);

  // Xử lý tạo nội dung bài viết bằng AI
  const handleGenerateContent = async () => {
    if (!aiPrompt.trim()) return;
    setIsGenerating(true);
    
    const systemInstruction = "Bạn là một biên tập viên xuất sắc chuyên về tài chính, crypto và tiếp thị liên kết (affiliate). Hãy viết một đoạn nội dung sắc bén, chuẩn SEO, giọng văn sang trọng và chuyên nghiệp dựa trên yêu cầu của người dùng. Trả về nội dung định dạng văn bản thuần túy hoặc Markdown, không kèm theo các câu giải thích dư thừa.";
    ...
    try {
      ...
      if (generatedText) {
        // Thêm nội dung AI tạo ra vào dưới nội dung hiện tại
        setContent(prev => prev + (prev ? '\n\n' : '') + generatedText);
        setAiPrompt('');
        setShowAIPrompt(false);
      }
    } catch (error) {
      // Bỏ qua log lỗi ra console theo nguyên tắc
    } finally {
      setIsGenerating(false);
    }
  };

  // Xử lý tạo Slug SEO bằng AI dựa trên Tiêu đề
  const handleGenerateSlug = async () => {
    if (!title.trim()) return;
    setIsGeneratingSlug(true);
    
    const payload = {
      contents: [{ parts: [{ text: `Tạo một URL slug chuẩn SEO bằng tiếng Việt không dấu, nối với nhau bằng dấu gạch ngang từ tiêu đề sau. Chỉ trả về kết quả slug, không thêm bất kỳ ký tự nào khác. Tiêu đề: "${title}"` }] }]
    };

    try {
      const result = await fetchGeminiWithRetry(payload);
      const generatedSlug = result?.candidates?.[0]?.content?.parts?.[0]?.text;
      
      if (generatedSlug) {
        setSlug(generatedSlug.trim().toLowerCase());
      }
    } catch (error) {
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm space-y-4">
            ...
            <div>
               <div className="flex items-center justify-between mb-2">
                 <label className="block text-sm font-medium text-slate-400">Nội dung (Rich Text / Markdown)</label>
                 
                 {/* Nút bật/tắt AI Assistant */}
                 <button 
                   onClick={() => setShowAIPrompt(!showAIPrompt)}
                   className={`text-xs font-medium px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all duration-300
                     ${showAIPrompt || isGenerating ? 'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-[0_0_10px_rgba(168,85,247,0.4)]' : 'bg-slate-800 text-purple-400 hover:bg-slate-700 border border-slate-700'}`}
                 >
                   <Sparkles size={14} className={isGenerating ? 'animate-pulse' : ''} /> 
                   {isGenerating ? 'AI đang viết...' : 'Trợ lý AI (Gemini)'}
                 </button>
               </div>
               
               {/* Khung nhập Prompt cho AI */}
               {showAIPrompt && (
                 <div className="mb-4 p-4 bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-purple-500/30 rounded-xl animate-in slide-in-from-top-2 duration-300">
                    <div className="flex gap-3">
                      <div className="flex-1 relative">
                        <Wand2 size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-purple-400/70" />
                        <input 
                          type="text" 
                          value={aiPrompt}
                          ...
                          placeholder="Ví dụ: Lập dàn ý cho bài viết đánh giá sàn Binance, hoặc Viết một đoạn mở bài cuốn hút..."
                          className="w-full bg-slate-950/80 border border-purple-500/30 rounded-lg pl-10 pr-4 py-2.5 text-sm text-purple-100 placeholder:text-purple-400/40 focus:outline-none focus:border-purple-400 shadow-inner"
                        />
                      </div>
                      <button 
                        onClick={handleGenerateContent}
                        disabled={isGenerating || !aiPrompt.trim()}
                        className="px-4 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:hover:bg-purple-600 text-white text-sm font-medium rounded-lg flex items-center gap-2 transition-colors"
                      >
                        {isGenerating ? <Loader2 size={16} className="animate-spin" /> : 'Tạo'}
                      </button>
                    </div>
                 </div>
               )}
               ...
               <div className="border border-slate-700 rounded-lg overflow-hidden bg-slate-950 flex flex-col h-[500px] shadow-inner">
                  <div className="bg-slate-800 px-4 py-2 border-b border-slate-700 flex gap-4 text-slate-400 items-center">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm space-y-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-sm text-slate-400">Đường dẫn (Slug)</label>
                <button 
                ...
                >
                  {isGeneratingSlug ? <Loader2 size={10} className="animate-spin"/> : <Sparkles size={10}/>} Tự tạo từ Tiêu đề
                </button>
              </div>
              <input 
                type="text" 
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm focus:border-cyan-500 focus:outline-none shadow-inner" 
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm space-y-4 relative overflow-hidden">
            ...
            <p className="text-xs text-slate-500 italic">Hệ thống sẽ tự động gắn tham số tracking ?sub_id={slug || 'slug'} khi xuất bản.</p>
    ...
    <div className="fixed inset-0 z-50 bg-[#0c0c0e] overflow-y-auto animate-in slide-in-from-bottom-10 duration-500">
      ...
      <article className="max-w-3xl mx-auto px-6 py-16 relative">
        ...
        <div className="prose prose-invert prose-lg max-w-none text-slate-300 leading-relaxed font-serif">
          ...
          <p>Nội dung chi tiết được trích xuất từ CMS: <br/><span className="italic text-slate-400">{article.content}</span></p>
...
const PublicHomeView = ({ onReadArticle, onGoToCms }) => {
  const publishedArticles = MOCK_ARTICLES.filter(a => a.status === 'published');
  const featuredArticle = publishedArticles[0]; // Giả lập bài đầu tiên là nổi bật
  const regularArticles = publishedArticles.slice(1);

  return (
    <div className="min-h-screen bg-[#060608] font-sans selection:bg-amber-500/30 text-slate-300 overflow-y-auto relative custom-scrollbar">
      {/* Background Decorators for Public View */}
      ...
      {/* Public Navigation */}
      <nav className="fixed w-full z-40 border-b border-white/5 bg-[#0a0a0c]/80 backdrop-blur-xl transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="text-2xl font-bold tracking-tighter text-white flex items-center gap-2 cursor-pointer">
            <span className="w-3 h-3 rounded-full bg-gradient-to-r from-amber-400 to-yellow-600 shadow-[0_0_10px_rgba(251,191,36,0.5)]"></span>
            NEXUS<span className="text-amber-400 font-light">FINANCE</span>
          </div>
          
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-400">
            <a href="#" className="text-white hover:text-amber-400 transition-colors">Trang chủ</a>
            <a href="#" className="hover:text-amber-400 transition-colors">Thị trường</a>
            <a href="#" className="hover:text-amber-400 transition-colors">Đánh giá Sàn</a>
            <a href="#" className="hover:text-amber-400 transition-colors">Kiến thức</a>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={onGoToCms}
              className="px-5 py-2 text-sm font-medium rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-white transition-all duration-300 flex items-center gap-2"
            >
              <User size={16}/> <span className="hidden sm:inline">Portal CMS</span>
            </button>
          </div>
        </div>
      </nav>
      ...
      <main className="relative z-10 pt-32 pb-20 px-6 max-w-7xl mx-auto space-y-20">
        ...
        {featuredArticle && (
          <section className="animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-amber-400 text-sm font-semibold tracking-widest uppercase">Tiêu điểm hôm nay</span>
            </div>
            
            <div 
              onClick={() => onReadArticle(featuredArticle)}
              className="group cursor-pointer relative rounded-[2rem] overflow-hidden bg-slate-900 border border-slate-800 hover:border-amber-500/50 transition-all duration-500 grid grid-cols-1 lg:grid-cols-2 shadow-2xl"
            >
              <div className="h-[300px] lg:h-[500px] relative overflow-hidden bg-slate-800">
                <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0c] via-transparent to-transparent z-10 lg:hidden"></div>
                {/* Giả lập hình ảnh */}
                <div className="w-full h-full bg-[url('https://images.unsplash.com/photo-1621761191319-c6fb62004040?q=80&w=2000&auto=format&fit=crop')] bg-cover bg-center group-hover:scale-105 transition-transform duration-700 opacity-60 mix-blend-luminosity"></div>
              </div>
              
              <div className="p-8 lg:p-16 flex flex-col justify-center relative z-20 bg-gradient-to-r from-[#0a0a0c] to-transparent lg:bg-[#0a0a0c]">
                <div className="flex items-center gap-3 text-xs text-slate-400 mb-6">
                  <span className="bg-amber-500/10 text-amber-400 px-3 py-1 rounded-full border border-amber-500/20">Phân tích</span>
                  <span>{featuredArticle.created_at}</span>
                  <span className="flex items-center gap-1"><Eye size={14}/> {featuredArticle.view_count.toLocaleString()}</span>
                </div>
                
                <h2 className="text-3xl lg:text-5xl font-bold text-white mb-6 leading-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-amber-200 group-hover:to-yellow-500 transition-all duration-300">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-white font-bold">
                    {MOCK_USERS.find(u => u.id === featuredArticle.author_id)?.avatar}
                  </div>
                  <div>
                    <p className="text-white font-medium">{MOCK_USERS.find(u => u.id === featuredArticle.author_id)?.name}</p>
                    <p className="text-xs text-slate-500">Chuyên gia phân tích</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="animate-in fade-in slide-in-from-bottom-12 duration-700 delay-300 fill-mode-both">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-2xl font-bold text-white">Tin tức & Phân tích mới</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {regularArticles.map((article, idx) => (
              <article 
                ...
                className="group cursor-pointer bg-slate-900/40 rounded-2xl border border-slate-800/60 overflow-hidden hover:border-amber-500/40 hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex flex-col"
              >
                <div className="h-48 bg-slate-800 relative overflow-hidden flex items-center justify-center">
                  <ImageIcon size={32} className="text-slate-700 group-hover:scale-110 transition-transform duration-500" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0c] to-transparent opacity-80"></div>
                  <div className="absolute top-4 left-4">
                    <span className="bg-black/50 backdrop-blur-md text-white text-[10px] uppercase tracking-wider px-3 py-1.5 rounded-full border border-white/10">
                      Crypto
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex items-center justify-between text-xs text-slate-500 mb-3">
                    <span>{article.created_at}</span>
                    <span className="flex items-center gap-1"><Eye size={14}/> {article.view_count.toLocaleString()}</span>
                  </div>
                  
                  <h4 className="text-xl font-bold text-white mb-3 leading-snug group-hover:text-amber-400 transition-colors line-clamp-2">
                    {article.title}
                  </h4>
                  ...
                  <div className="pt-4 border-t border-slate-800/60 flex items-center justify-between">
                     <span className="text-sm font-medium text-slate-300">
                       {MOCK_USERS.find(u => u.id === article.author_id)?.name}
                     </span>
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800/60 bg-[#0a0a0c]/80 py-12 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-xl font-bold tracking-tighter text-slate-600 flex items-center gap-2">
            NEXUS<span className="font-light">FINANCE</span>
          </div>
          <p className="text-xs text-slate-500 text-center md:text-left">
            © 2026 Nexus Finance LLC. Tòa nhà Bitexco, Q1, TP.HCM.<br/>
            Cảnh báo rủi ro: Giao dịch Crypto mang lại rủi ro cao, vui lòng quản lý vốn nghiêm ngặt.
          </p>
        </div>
      </footer>
    </div>
...
export default function App() {
  const [appMode, setAppMode] = useState('public'); // 'public' | 'cms'
  ...
  const renderContent = () => {
    // Nếu đang có bài viết preview được chọn, luôn render lớp đè Preview
    if (previewArticle) return <PublicArticlePreview article={previewArticle} onBack={() => setPreviewArticle(null)} />;
    
    // Nếu đang ở mode Public
    if (appMode === 'public') {
      return <PublicHomeView onReadArticle={setPreviewArticle} onGoToCms={() => setAppMode('cms')} />;
    }
  ...
  return (
    <div className="min-h-screen bg-[#060608] font-sans selection:bg-amber-500/30 text-slate-300">
      ...
      {previewArticle === null && appMode === 'cms' && (
          ...
          <main className="flex-1 flex flex-col overflow-hidden">
            ...
            <header className="h-20 bg-[#0a0a0c]/50 backdrop-blur-md border-b border-slate-800/60 flex items-center justify-between px-8 relative z-20">
               ...
               <div className="flex items-center gap-6">
                 ...
                 {/* Nút chuyển về Public FE */}
                 <button 
                   onClick={() => setAppMode('public')}
                   className="hidden md:flex items-center gap-2 text-xs font-medium bg-white/5 hover:bg-white/10 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700 transition-colors"
                 >
                   <Globe size={14}/> Trở về Website
                 </button>
