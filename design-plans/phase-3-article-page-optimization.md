# Build a Homepage-Aligned Reading Room for Individual Articles
...
- Problem: the article route previously introduced a separate dark-green reading theme while the homepage establishes a light editorial canvas, white surfaces, black ink, and teal signal accents. That split made route transitions feel like separate products and encouraged low-contrast mint/teal combinations. The article still needs a 65–75-character measure, rich blockquote/link/code treatments, sticky contents and sponsor rail, explicit author/read metadata, top-aligned sharing, and a bottom continuation grid.
...
Use the homepage’s `#F5F5F2` editorial canvas, white reading surfaces, `#111111` ink, `#374151` copy, and `#0D766E` accessible action accent. Align the masthead to the same full-width grid as the lead thumbnail: a restrained title on the dominant left track, a top-aligned “In brief” introduction on the quieter right track, and metadata spanning both tracks with a compact vertical rhythm. Collapse that composition to one column below tablet width, where a horizontal rule replaces the desktop divider. Keep the lead image broad for editorial impact, then narrow the prose to `72ch` and pair it with a sticky rail. The rail begins with a client-derived table of contents whose active teal marker acts as the page’s reading signal, followed by the existing affiliate owner in a homepage-aligned light variant. Put sharing in the masthead, add a publication-safe author panel after the prose, and move related stories into a full-width bottom grid. Reserve the near-black surface for semantically distinct anchors such as code and global navigation/footer, with mint used only where it has sufficient contrast.

## Reuse

- `--editorial-ink`, `--editorial-copy`, `--editorial-muted`, `--editorial-divider`, `--ed-verdict`, and the Phase 2 mint `#85DDD5`.
- Existing `EditorialHeader`, `EditorialFooter`, `EditorialBackdrop`, `PublicArticleImage`, `ArticleContent`, `SocialShare`, `VerticalAffiliateSidebar`, `EditorVerdict`, `AffiliateCtaBlock`, and `StickyMobileBar` behaviors and homepage surface tokens.
...
2. `src/app/article/[slug]/article.module.css`
   - Change: implement the homepage-aligned light article shell, thumbnail-aligned two-track masthead, 72ch reading measure, hierarchical heading scale, 1.82 body leading, rich blockquote/link/code/table/media styles, multi-column desktop grid, sticky rail, author panel, and responsive bottom grid.
...
3. `src/components/ArticleTableOfContents.tsx` and `src/components/ArticleTableOfContents.module.css`
   - Change: add a client component that discovers sanitized H2/H3 headings, assigns stable unique IDs, tracks the active section with `IntersectionObserver`, and renders a compact light anchored outline.
...
5. `src/components/SocialShare.tsx` and `src/components/SocialShare.module.css`
   - Change: add a compact masthead variant with icon-forward homepage surface buttons while retaining the current full variant as the default.
...
6. `src/components/VerticalAffiliateSidebar.tsx`
   - Change: add an `article` variant with white cards, black actions, and teal interaction states for the article rail; keep the default collection behavior unchanged.
...
- System: confirm the article uses existing display/body fonts, the 1440px rail, shared homepage surface tokens, and accessible teal-on-light or mint-on-dark accents rather than parallel route themes.
