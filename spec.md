# BẢN ĐẶC TẢ KỸ THUẬT VÀ KIẾN TRÚC THÔNG TIN (V4.0 EXTENDED)

**Phiên bản:** 4.0 Extended  
...
Hệ thống sử dụng các bảng chuẩn hóa để giải quyết triệt để bài toán: 1 bài viết chứa nhiều nhóm danh mục (Categories/Sub-categories), chứa nhiều nút Affiliate khác nhau ở các vị trí khác nhau (`position_label`), và phân quyền tuyệt đối giữa Admin và Editor/Author.
...
| `name` | TEXT | Họ và tên hiển thị tác giả. |
| `status` | TEXT | Trạng thái tài khoản: `'active'` hoặc `'inactive'`. |
| `avatar` | TEXT | Ảnh/chữ cái đại diện. |

### 1.2. Bảng `categories` & `sub_categories` (Phân nhóm danh mục đa cấp)
Để giải quyết bài toán chia nhóm nhỏ (Ví dụ: Danh mục lớn "Tài chính", nhóm nhỏ "Quỹ mở", "Crypto"), ta tách thành bảng Danh mục cha và Danh mục con.
| Bảng & Tên trường | Kiểu dữ liệu | Mô tả chi tiết |
| :--- | :--- | :--- |
| **`categories`** (`id`, `name`, `slug`) | INTEGER, TEXT, TEXT | Danh mục cấp 1 (Ví dụ: ID 1 - Tài chính, ID 2 - Bất động sản, ID 3 - Công nghệ). |
| **`sub_categories`** (`id`, `category_id`, `name`, `slug`) | INTEGER, INTEGER, TEXT, TEXT | Danh mục cấp 2 thuộc cấp 1 (Ví dụ: Thuộc Tài chính có Quỹ mở, Crypto, Chứng khoán). |
...
| `category_id` | INTEGER | Khóa ngoại liên kết `categories.id` (Danh mục lớn). |
| `sub_category_id` | INTEGER | Khóa ngoại liên kết `sub_categories.id` (Danh mục con). |
...
| `revenue` | REAL | Doanh thu ước tính $ (Default: 0). |
| `meta_title` / `meta_description` | TEXT | Thẻ SEO On-page tối ưu tìm kiếm. |
| `thumbnail_url` | TEXT | URL ảnh đại diện bài viết. |
...
| `commission` | TEXT | Mức hoa hồng (Ví dụ: 50,000 VND / Lead hoặc 40% Fee). |
| `cookie` | TEXT | Thời hạn cookie (Ví dụ: 30 ngày, 90 ngày). |
...
### 1.7. Bảng `settings` (Cấu hình SEO & GEO System)
| Tên trường (Field) | Kiểu dữ liệu | Mô tả chi tiết |
| :--- | :--- | :--- |
| `id` | INTEGER | Khóa chính (Default: 1). |
| `site_title`, `meta_description`, `focus_keywords`, `canonical_url` | TEXT | Cấu hình SEO On-page tập trung. |
| `hreflang`, `geo_target`, `business_name`, `business_address`, `business_phone` | TEXT | Định danh GEO Local SEO & NAP Business Info. |
| `og_image_url`, `schema_jsonld`, `head_scripts` | TEXT | Social OpenGraph Image & Schema JSON-LD Scripts. |
...
| **Gắn Link vào Bài viết** | Được chọn bất kỳ link nào từ kho link gốc để cấu hình nhiều vị trí trong bài viết kèm nhãn `position_label`. | Chỉ được chọn các link có sẵn trong kho do Admin cung cấp để chèn vào bài của mình, không sửa được URL gốc. |
...
* **Tab "Bài viết Hot Tuần":** Lọc các bài viết có trường `is_featured = TRUE` kết hợp lượng tương tác cao trong 7 ngày gần nhất.
* **Tab "Bài viết Xem Nhiều":** Sắp xếp danh sách theo trường `view_count DESC` giảm dần.
* **Tab "Bài viết Mới Nhất":** Sắp xếp theo trường `created_at DESC`.
