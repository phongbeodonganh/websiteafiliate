...
- **SEO-02 — Structured data:** every article page auto-embeds `NewsArticle` JSON-LD (headline, image, datePublished, author, publisher "AI AFFILIATE HUB") with canonical exactly `https://aiaffiliatehub.com/article/[slug]`. [V5.2 §2.2] — status: **verify**
...
- **SEO-04 — Settings-driven site metadata:** the settings singleton drives site title, meta description, focus keywords, canonical, hreflang/geo target, NAP, OG image, JSON-LD, head scripts (V4 field list), configured to the V5.2 §1.7 values (site_title "AI AFFILIATE HUB", canonicalUrl aiaffiliatehub.com, geoTarget "US, VN, GLOBAL", primary #0056B3, accent #FF6B6B). [V5.2 §1.7; V4 standing] — status: **partial** (mechanism exists; current defaults say aidealsuk.com)
